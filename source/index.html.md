...
Оформление покупки совершается аналогичным образом с помощью метода <a href="#registration">Checkout</a>, который возвращает ссылку на iFrame Рево, но уже с большим числом шагов. После регистрации (или авторизации) клиента, появляется шаг с выбором графика платежей. Принципиальная схема процесса представлена на диграмме справа.
...
:-|:-|:-
Отправить запрос с помощью метода <a href="#registration">auth</a> | Получить ссылку на вызов iframe| status: Pending; decision: null
Вызов iframe_url | Отображение <a href="#ee363a7d6e">iframe формы</a> с вводом персональных данных| status: Pending; decision: null
Ввод несуществующих в базе данных и нажатие кнопки «Оформить» | Переход на этап подтверждения СМС-кодом | status: Pending; decision: null
Нажатие на кнопку «Подтвердить» | Отображение экрана результата с информацией об одобренном лимите | status: xxx; decision: approved
...
Вызов iframe_url | Отображение <a href="#ee363a7d6e">iframe формы</a> с вводом персональных данных| status: Pending; decision: null
Ввод несуществующих в базе данных и нажатие кнопки «Оформить» | Переход на этап подтверждения СМС-кодом | status: Pending; decision: null
Нажатие на кнопку «Подтвердить» | Отображение экрана результата с информацией о недоступности услуги оплаты частями | status: xxx; decision: rejected
...
Вызов iframe_url | Отображение <a href="#ee363a7d6e">iframe формы</a> с вводом персональных данных| status: Pending; decision: null
Ввод несуществующих в базе данных и нажатие кнопки «Оформить» | Переход на этап подтверждения СМС-кодом | status: Pending; decision: null
Нажатие кнопки изменения номера на экране подтверждения СМС-кодом | Переход на этап ввода номера телефона | status: Pending; decision: null
Ввод несуществующего в базе номера и нажатие кнопки «Отправить SMS» | Переход на этап подтвержения СМС-кодом | status: Pending; decision: null
